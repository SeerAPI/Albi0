...
def bundle_bytes_struct(writer: Writer, schema: BytesStructSchema, tag: str = "") -> bytes:
    """
    将结构化数据打包成字节数组
    ...
    for v in schema:
        if isinstance(v, bool):
            ...
            bytes_list.extend(bundle_string_with_length(
                v, GlobalLengthType.value))
    ...
    if tag:
        print(f"{tag}: total length: {bundle_length}")
